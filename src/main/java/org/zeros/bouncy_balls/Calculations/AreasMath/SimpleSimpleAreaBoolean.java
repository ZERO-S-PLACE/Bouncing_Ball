...
public class SimpleSimpleAreaBoolean extends AreaBoolean {
    ...
    private final ArrayList<Point2D> intersections = new ArrayList<>();
    private Area areaA;
    private Area areaB;
        ...
        for (Point2D intersection : intersections) {
            ...
            for (Segment startSegment : segmentsAtIntersection) {
                ...
                if (isAtomicArea(newArea, subAreas)) {
                    subAreas.removeIf(area -> AreasMath.isInsideArea(area, newArea.getPointInside()) && AreasMath.isInsideArea(area, newArea.getCorners().getFirst()));
            ...
            for (Segment segment1 : area1segments) {
                for (Segment segment2 : area2segments) {
                    ...
                    for (Point2D intersection : tempIntersections) {
                        ...
                        if (VectorMath.doesNotContainPoint(intersection, intersections)) {
